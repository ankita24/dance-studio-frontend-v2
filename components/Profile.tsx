...
export default function Profile({
  ...
  const fetchProfile = () => {
    axios
      .get(`http://192.168.29.91:9999/api/profile/${studioId}`)
      .then(response => {
  ...
  const SaveDetails = () => {
    if (studioId) {
      axios
        .put(`http://192.168.29.91:9999/api/owner/${studioId}`, editableData)
        .then(res => {
    ...
    <View style={styles.container}>
      ...
      {profile ? (
        <View style={{ padding: 10 }}>
          ...
          <Text style={[styles.marginBottom20, styles.textSize]}>
            <Text style={[styles.fontWeight, styles.textSize]}>Location:</Text>
            {profile.location}
          </Text>
          <View style={[styles.marginBottom20, styles.flex]}>
